...
# FROM            계정/eb_trello
FROM            yykdev/eb_trello
MAINTAINER      assavictory@gmail.com

# 현재경로의 모든 파일들을 컨테이너의 /srv/trello 폴더에 복사
COPY            . /srv/trello

# cd /srv/trello와 같은 효과
WORKDIR         /srv/trello

# requirements 설치 ( requirements.txt 에 기록해 둔 pip list 를 모두 install )
RUN             /root/.pyenv/versions/trello/bin/pip install -r .requirements/requirements.txt
...
# collectstatic 실행
#RUN             /root/.pyenv/versions/trello/bin/python /srv/trello/django_app/manage.py collectstatic --settings=config.settings.deploy --noinput
...
#   home에 가상환경 적용후 실행
# /root/.pyenv/versions/trello/bin/uwsgi --http :8000 --chdir /srv/trello/django_app --home /root/.pyenv/versions/trello -w config.wsgi.debug
#
# 아래는 위 내용을 줄바꿈 처리
#
# /root/.pyenv/versions/trello/bin/uwsgi \
# --http :8000 \
# --chdir /srv/trello/django_app \
# --home /root/.pyenv/versions/trello -w config.wsgi.debug
#
#
